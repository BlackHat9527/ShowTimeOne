...
class ChatServiceServices extends BaseServices
{
    ...
    {
        // 如果有token则查询该用户所在视讯服务器信息
        file_put_contents("/www/wwwroot/chat/log.txt", PHP_EOL . config('app.gamehost') . ' 请求地址', FILE_APPEND);
        if($user['authorization']){
            $header = array(
                'Authorization:' . $user['authorization']
            );
            $url = config('app.gamehost') ."/api/lgUser/getUserInfo";
            $data = $this->curlGet($url, $header);
            $data = \Qiniu\json_decode($data, true);
            file_put_contents("/www/wwwroot/chat/log.txt", PHP_EOL . json_encode($data), FILE_APPEND);
            ...
            $url = config('app.gamehost') ."/api/lgUser/getUserInfoEx?uuid=" . $user['userUuid'];
            $data = $this->curlGet($url, null);
            $data = \Qiniu\json_decode($data, true);
            file_put_contents("/www/wwwroot/chat/log.txt", PHP_EOL . json_encode($data), FILE_APPEND);
            if($data && $data['code'] == 200){
                $data = $data['data'];
        ...
        file_put_contents("/www/wwwroot/chat/log.txt", PHP_EOL . json_encode($user), FILE_APPEND);
        $uid = $user['uid'] ?? 0;
        ...
        if (!$uid || !$userInfo) {
            ...
            if (isset($user['phone']) && $user['phone'] && $user['phone'] != $userInfo->phone) {
                $save = true;
        ...
        //获取当前分配客服,必须在线的,且有效的
        $toUserId = $this->dao->count(['appid' => $appId, 'status' => 1, 'online' => 1, 'user_id' => $toUserId]) ? $toUserId : 0;
        if (!$toUserId && $kefuId) {
            $toUserId = $this->dao->value(['appid' => $appId, 'status' => 1, 'online' => 1, 'id' => $kefuId], 'user_id');
        }
        //随机客服
